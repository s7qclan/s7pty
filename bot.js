...
client.on("guildMemberAdd", member => {
      ...
      const welcomer = member.guild.channels.find("name","quiet"); //اسم روم
   ...
   ctx.fillText(` Welcome to ${member.guild.name} server` , 200, 190);
    ...
    welcomer.sendFile(canvas.toBuffer());
	  ...
	  client.on("guildMemberAdd", member => {
let welcomer = member.guild.channels.find("name","quiet");
      if(!welcomer) return;
      if(welcomer) {
         moment.locale('ar-ly');
         var h = member.user;
...
.addField ('**وعليكم السلام ورحمة الله وبركاتة منور:sparkling_heart: **', ' ! ')
.setColor('#7d2dbe')
  message.channel.sendEmbed(embed);
    ...
    client.on('message', message => {
     ...
     let embed = new Discord.RichEmbed()
.setThumbnail(message.author.avatarURL)
.addField ('**اطلق نقطه عرفها التاريخ :sunglasses:**', ' ! ')
.setColor('#7d2dbe')
  message.channel.sendEmbed(embed);
            }
 });
...
client.on("guildMemberAdd", member => {
  ...
  return channel.send(`**__Welcome To Quiet__**`)
}).catch(console.error)
})
...
client.on('ready', () => {
  console.log(`Logged in as ${client.user.tag}!`);
  client.user.setGame(`Quiet System`,"http://twitch.tv/vAmmvr_");
